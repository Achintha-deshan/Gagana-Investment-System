...
class CustomerService {
    ...
    async generateNextCustomerId() {
        try {
            ...
            // C පසුව ඉලක්කම් 4ක දිගක් පවත්වා ගනී (උදා: C0002)
    ...
    /**
     * 3. අලුත් පාරිභෝගිකයෙකු ඇතුළත් කිරීම (Gender සමඟ)
     */
    async add(customerData) {
        try {
            ...
            const sql = `
                ...
                (CustomerID, CustomerName, NIC, CustomerPhone, CustomerAddress, Gender, IsBlacklisted, BlacklistReason) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    ...
    /**
     * 4. පාරිභෝගික තොරතුරු යාවත්කාලීන කිරීම (Gender සමඟ)
     */
        ...
        try {
            ...
            const sql = `
                ...
                SET CustomerName = ?, NIC = ?, CustomerPhone = ?, CustomerAddress = ?, 
                    Gender = ?, IsBlacklisted = ?, BlacklistReason = ? 
                WHERE CustomerID = ?`;
    ...
    async delete(customerId) {
        ...
        try {
            ...
            if (loans.length > 0) {
                const loanIds = loans.map(l => l.LoanID); // මෙහි L.LoanID ලෙස නිවැරදි විය යුතුයි

                // 2. ණය වලට අදාළ සියලුම දත්ත මැකීම
